...
def gradient_descent(f, x0, steps, obj_tol, param_tol, max_iter):
    ...
    return {'res' : flag_conv, 'path_to': val_y}

def wolfe_searching_steps(x_target, p_target, c_1, a_init, f, gradient, factor_backtracking):
    check_obj = False
    a_update = a_init
    count = 0
    #loop to update.
    while check_obj is False:
        ...
        #update
        a_update = factor_backtracking * a_update
...
def search_lin(f, x0, steps, obj_tol, param_tol, max_iter, select_dir, step_from=1.0,
                c_1=0.01, factor_backtracking=0.5):
    x=x0
    flag_conv = False
    x_prev = np.inf
    y_prev = np.inf
    val_y = []
    p_target = p_prev = None

    for i in range(max_iter):
        #init_instance
        [y_pres, gradient, hes] = f(x, True)
        val_y.append({'ind':  x, 'val': y_pres})
        ...
        if i == 0:
            ...
            a_update = wolfe_searching_steps(p_prev, p_prev, c_1, step_from, f, gradient, factor_backtracking)
